 ...
 *         for ST-Ericsson.
 *
 * Modified by cocafe@xda-developers.com
 *
 * License terms:
 *
...
int hsx_read;
int hsx_write = 0;
bool hsx_boost = false;
module_param(hsx_read, int, 0444);
module_param(hsx_write, int, 0644);
module_param(hsx_boost, bool, 0644);
...
void ab8500_audio_power_control(bool power_on)
{
	...
	if (hsx_boost && power_on) {	
		//snd_soc_write(ab8500_codec, REG_ANAGAIN3, REG_ANAGAIN3_BOOST);
		snd_soc_write(ab8500_codec, REG_ANAGAIN3, hsx_write); /* Now control by hsx_write */
	}

	hsx_read = snd_soc_read(ab8500_codec, REG_ANAGAIN3);
	printk("ab8500_asoc: REG_ANAGAIN3 = %d \n", hsx_read);
}
